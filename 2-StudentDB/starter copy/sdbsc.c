...
int get_student(int fd, int id, student_t *s){
    ...
    // try to locate the student based off ID (must be in domain)
    if (! (id>=MIN_STD_ID && id<=MAX_STD_ID)){
        return SRCH_NOT_FOUND;
    }

    // if the ID is valid we can search using the ID
    ...
    // if position found, we want to read/copy it
    ...
    if (read(fd, s, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
        ...
        // lastly check if record is valid
        if (s->id == 0){
                return SRCH_NOT_FOUND;
        } //else {
                //return NO_ERROR;
...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    // check ID and GPA are in domain
    if (validate_range(id, gpa) != NO_ERROR) {
        printf(M_ERR_STD_RNG);
        return ERR_DB_OP;
    }

    // if valid, search location
    int offset = id * STUDENT_RECORD_SIZE;
    if (lseek(fd, offset, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    ...
    // if position found, check if student already exists
    student_t student;
    ...
    // go back to original offset location
    if (lseek(fd, offset, SEEK_SET) == -1) {
...
int count_db_records(int fd){
    // start at beginning
        ...
        // read individual records until EOF (0)
        student_t student;
        ssize_t bytes_read;
        int count = 0;
        ...
        // check if error read + return # of record
        if (bytes_read == -1){
                printf(M_ERR_DB_READ);
                return ERR_DB_FILE;
        }
...
void print_student(student_t *s){
    // ensure student pointer is valid
    if (s == NULL || s->id == 0) {
    printf(M_ERR_STD_PRINT);
        return;
    }

    // first print header then student data
    printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA");
...
int compress_db(int fd) {
    ...
    if (lseek(fd, 0, SEEK_SET) == -1) {
        perror("Error seeking in db file");
        close(temp_fd);
        return ERR_DB_FILE;
    }
    ...
    if (remove(DB_FILE) == -1) {
        perror("Error removing old db file");
        return ERR_DB_FILE;
    }
    ...
    if (new_fd == -1) {
        perror("Error opening new db file");
        return ERR_DB_FILE;
    }
...
//     // Seek to the start of the database file
//     if (lseek(fd, 0, SEEK_SET) == -1) {
//         printf(M_ERR_DB_READ);
//         close(temp_fd);
//         return ERR_DB_FILE;
...
//     student_t student;
//     ssize_t bytes_read, bytes_written;
//     off_t valid_records = 0; // Keep track of how many valid records exist

//     // Read and copy valid records to the temp file
//     while ((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE) {
//         if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
//             bytes_written = write(temp_fd, &student, STUDENT_RECORD_SIZE);
//             if (bytes_written != STUDENT_RECORD_SIZE) {
...
//     // Close both files before replacing the old database
//     close(fd);
//     close(temp_fd);
...
//     // Rename temp file to become the new database
//     if (rename(TMP_DB_FILE, DB_FILE) == -1) {
//         printf(M_ERR_DB_CREATE);
//         return ERR_DB_FILE;
//     }

//     // Reopen the newly compressed database file and return its file descriptor
//     int new_fd = open(DB_FILE, O_RDWR);
//     if (new_fd == -1) {
//         printf(M_ERR_DB_OPEN);
//         return ERR_DB_FILE;
...
//     // If no valid records were found, truncate file to 0 bytes
//     if (valid_records == 0) {
//         ftruncate(new_fd, 0);
//     }
...
//     // Read records from the old database
//     while ((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE) {
//         // Check if the record is valid (not empty)
//         if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
...
//     // Rename temporary database to the original database name
//     if (rename(TMP_DB_FILE, DB_FILE) == -1) {
//         printf(M_ERR_DB_CREATE);
//         return ERR_DB_FILE;
//     }

//     // Open the new database file and return its file descriptor
//     int new_fd = open(DB_FILE, O_RDWR);
//     if (new_fd == -1) {
//         printf(M_ERR_DB_OPEN);
//         return ERR_DB_FILE;
